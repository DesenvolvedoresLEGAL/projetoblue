...
export const useAssociationActions = () => {
  ...
  const endAssociation = useMutation({
    mutationFn: async ({
      ...
      console.log("🔧 useAssociationActions - Ending association:", {
        associationId,
        assetId,
      });
  ...
  const bulkEndAssociations = useMutation({
    mutationFn: async (
      associations: Array<{ id: number; asset_id: string }>
    ) => {
      console.log(
        "🔧 useAssociationActions - Bulk ending associations:",
        associations
      );
      ...
      if (errors.length > 0) {
        ...
        toast.warning(
          `${results.length} associações encerradas, ${errors.length} falharam`
        );
      } else {
        toast.success(
          `Todas as ${results.length} associações foram encerradas com sucesso!`
        );
      }
