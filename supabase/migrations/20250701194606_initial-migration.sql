...
CREATE TYPE "public"."asset_type_enum" AS ENUM ('chip', 'equipment');

ALTER TYPE "public"."asset_type_enum" OWNER TO "postgres";

CREATE TYPE "public"."association_type_enum" AS ENUM ('aluguel', 'assinatura');
...
CREATE
OR REPLACE FUNCTION "public"."acquire_operation_lock"(
    "p_operation_type" "text",
    "p_resource_id" "text",
    "p_operation_data" "jsonb" DEFAULT NULL :: "jsonb",
    "p_timeout_minutes" integer DEFAULT 5
) RETURNS "jsonb" LANGUAGE "plpgsql"
SET
    "search_path" TO 'public, auth' AS $ $ DECLARE lock_id UUID;

existing_lock RECORD;

current_user_id UUID;

BEGIN -- Log dos parâmetros de entrada
...
OR trim(p_operation_type) = '' THEN RETURN jsonb_build_object(
    'acquired',
    false,
    'error_code',
    'INVALID_PARAMETERS',
    'message',
    'operation_type é obrigatório'
);

END IF;

IF p_resource_id IS NULL
OR trim(p_resource_id) = '' THEN RETURN jsonb_build_object(
    'acquired',
    false,
    'error_code',
    'INVALID_PARAMETERS',
    'message',
    'resource_id é obrigatório'
);
...
RAISE NOTICE '[acquire_operation_lock] User ID atual: %',
current_user_id;
...
IF existing_lock.id IS NOT NULL THEN RAISE NOTICE '[acquire_operation_lock] Lock existente encontrado: %, owner: %',
existing_lock.id,
...
RAISE NOTICE '[acquire_operation_lock] Lock renovado para user %',
current_user_id;
...
ELSE -- Lock de outro usuário ainda ativo
...
RETURN jsonb_build_object(
    ...
    'RESOURCE_LOCKED',
    'message',
    'Recurso está sendo usado por outro usuário',
    'lock_owner',
    existing_lock.user_id,
    'expires_at',
...
RAISE NOTICE '[acquire_operation_lock] Novo lock criado: %',
lock_id;
...
EXCEPTION
WHEN OTHERS THEN RAISE NOTICE '[acquire_operation_lock] ERRO: % (SQLSTATE: %)',
SQLERRM,
SQLSTATE;
...
ALTER FUNCTION "public"."acquire_operation_lock"(
    "p_operation_type" "text",
    "p_resource_id" "text",
...
CREATE
OR REPLACE FUNCTION "public"."admin_delete_user"("user_id" "uuid") RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE user_email text;
...
CREATE
OR REPLACE FUNCTION "public"."admin_list_users"() RETURNS TABLE(
    "id" "uuid",
    "email" "text",
...
SET
    "search_path" TO 'public' AS $ $ BEGIN -- Check if the current user is admin
    IF NOT (
        SELECT
            has_role('admin' :: user_role_enum)
    ) THEN RAISE EXCEPTION 'Access denied: only administrators can list users';
...
CREATE
OR REPLACE FUNCTION "public"."admin_update_user_profile"("user_id" "uuid", "profile_updates" "jsonb") RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE old_data jsonb;
...
IF NOT (
    SELECT
        has_role('admin' :: user_role_enum)
) THEN RAISE EXCEPTION 'Acesso negado: apenas administradores podem atualizar perfis';

END IF;

-- Obter dados atuais do usuário
SELECT
    row_to_json(profiles.*),
    email INTO old_data,
    user_email
FROM
    profiles
...
EXCEPTION
WHEN OTHERS THEN RAISE EXCEPTION 'Erro ao atualizar perfil: %',
SQLERRM;
...
CREATE
OR REPLACE FUNCTION "public"."admin_update_user_role"(
    "user_id" "uuid",
    "new_role" "public"."user_role_enum"
) RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE old_role user_role_enum;
...
IF NOT (
    SELECT
        has_role('admin' :: user_role_enum)
) THEN RAISE EXCEPTION 'Acesso negado: apenas administradores podem alterar roles';
...
IF old_role = new_role THEN RETURN TRUE;
...
EXCEPTION
WHEN OTHERS THEN RAISE EXCEPTION 'Erro ao atualizar role: %',
SQLERRM;
...
ALTER FUNCTION "public"."admin_update_user_role"(
    "user_id" "uuid",
    "new_role" "public"."user_role_enum"
) OWNER TO "postgres";

CREATE
OR REPLACE FUNCTION "public"."check_asset_availability"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
...
IF EXISTS (
    ...
    WHERE
        ...
        AND (
            NEW.entry_date < COALESCE(exit_date, 'infinity')
            AND COALESCE(NEW.exit_date, 'infinity') > entry_date
        )
) THEN RAISE EXCEPTION 'O ativo % já está associado a outro cliente neste período.',
NEW.asset_id;
...
CREATE
OR REPLACE FUNCTION "public"."cleanup_expired_locks"() RETURNS integer LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE deleted_count INTEGER;
...
CREATE
OR REPLACE FUNCTION "public"."detect_association_inconsistencies"() RETURNS TABLE(
    "asset_id" "text",
...
SET
    "search_path" TO 'public' AS $ $ DECLARE status_disponivel_id BIGINT;
...
BEGIN -- Buscar IDs de status
SELECT
    id INTO status_disponivel_id
FROM
    asset_status
WHERE
    LOWER(status) IN ('disponível', 'disponivel')
LIMIT
    1;

SELECT
    id INTO status_alocado_id
FROM
    asset_status
WHERE
    LOWER(status) = 'alocado'
LIMIT
    1;
...
FROM
    assets a
    LEFT JOIN asset_client_assoc aca ON a.uuid = aca.asset_id
    AND aca.exit_date IS NULL
WHERE
    aca.asset_id IS NULL
    AND a.status_id != status_disponivel_id
    AND a.deleted_at IS NULL LOOP -- Corrigir o status
UPDATE
    assets
SET
    status_id = status_disponivel_id
WHERE
    uuid = inconsistency_record.asset_uuid;
...
VALUES
    (
        ...
        jsonb_build_object(
            'asset_id',
            inconsistency_record.asset_uuid,
            'correction_type',
            'status_fix',
            'reason',
            'Asset had no active associations but wrong status',
            'corrected_by',
...
VALUES
    (
        ...
        jsonb_build_object(
            ...
            'status_fix',
            'reason',
            'Asset had active association but wrong status',
            'corrected_by',
            'detect_association_inconsistencies'
        ),
        inconsistency_record.current_status,
...
RAISE NOTICE 'Inconsistency detection completed. % corrections made.',
correction_count;
...
CREATE
OR REPLACE FUNCTION "public"."ensure_user_profile"(
    "user_id" "uuid",
    "user_email" "text",
    "user_role" "public"."user_role_enum"
) RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ BEGIN -- Try to insert the profile if it doesn't exist
INSERT INTO
    public.profiles (id, email, role, is_active, is_approved)
VALUES
    (user_id, user_email, user_role, true, true) ON CONFLICT (id) DO
UPDATE
SET
...
EXCEPTION
WHEN OTHERS THEN RAISE WARNING 'Error ensuring profile for %: %',
user_email,
SQLERRM;
...
ALTER FUNCTION "public"."ensure_user_profile"(
    "user_id" "uuid",
    "user_email" "text",
    "user_role" "public"."user_role_enum"
) OWNER TO "postgres";
...
CREATE
OR REPLACE FUNCTION "public"."fix_missing_profiles"() RETURNS TABLE(
    "user_id" "uuid",
    "email" "text",
    "fixed" boolean
) LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO '' AS $ $ DECLARE u RECORD;
...
RAISE NOTICE 'Perfil criado com sucesso para usuário %',
u.email;

RETURN NEXT;

EXCEPTION
WHEN OTHERS THEN error_count := error_count + 1;
...
RAISE NOTICE 'Erro ao criar perfil para %: %',
u.email,
SQLERRM;
...
RAISE NOTICE 'Resumo: % perfis criados, % erros',
fixed_count,
error_count;
...
SET
    "search_path" TO 'public' AS $ $ DECLARE user_role user_role_enum;

BEGIN -- Log de início da função
RAISE NOTICE 'handle_new_user: Iniciando criação de perfil para usuário % (email: %)',
new.id,
new.email;

-- Tentar converter o role dos metadados do usuário
BEGIN user_role := (new.raw_user_meta_data ->> 'role') :: user_role_enum;

RAISE NOTICE 'handle_new_user: Role extraído dos metadados: %',
user_role;

EXCEPTION
WHEN invalid_text_representation THEN -- Usar 'cliente' como padrão se a conversão falhar
user_role := 'cliente' :: user_role_enum;

RAISE NOTICE 'handle_new_user: Erro ao converter role, usando padrão "cliente" para usuário %',
new.email;

WHEN OTHERS THEN -- Fallback para qualquer outro erro
user_role := 'cliente' :: user_role_enum;

RAISE NOTICE 'handle_new_user: Erro inesperado ao processar role, usando padrão "cliente" para usuário %',
new.email;
...
RAISE NOTICE 'handle_new_user: Perfil criado com sucesso para usuário % com role %',
new.email,
COALESCE(user_role, 'cliente' :: user_role_enum);

EXCEPTION
WHEN unique_violation THEN RAISE NOTICE 'handle_new_user: Perfil já existe para usuário %',
new.email;

WHEN OTHERS THEN RAISE WARNING 'handle_new_user: Erro ao criar perfil para %: % (SQLSTATE: %)',
new.email,
SQLERRM,
SQLSTATE;
...
CREATE
OR REPLACE FUNCTION "public"."has_minimum_role"("required_role" "public"."user_role_enum") RETURNS boolean LANGUAGE "plpgsql" STABLE SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE user_role user_role_enum;
...
CREATE
OR REPLACE FUNCTION "public"."has_role"("role_name" "public"."user_role_enum") RETURNS boolean LANGUAGE "sql" STABLE SECURITY DEFINER
SET
...
CREATE
OR REPLACE FUNCTION "public"."is_afiliado"() RETURNS boolean LANGUAGE "sql" STABLE SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $
SELECT
    has_role('cliente' :: user_role_enum);
...
CREATE
OR REPLACE FUNCTION "public"."is_support_or_above"() RETURNS boolean LANGUAGE "sql" STABLE SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $
SELECT
    has_minimum_role('suporte' :: user_role_enum);
...
CREATE
OR REPLACE FUNCTION "public"."is_user_self"("profile_id" "uuid") RETURNS boolean LANGUAGE "sql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $
SELECT
    profile_id = auth.uid() $ $;
...
CREATE
OR REPLACE FUNCTION "public"."log_and_update_status"() RETURNS "trigger" LANGUAGE "plpgsql"
SET
    "search_path" TO 'public, auth' AS $ $ DECLARE status_antigo BIGINT;
...
SELECT
    solution INTO solution_name
FROM
    asset_solutions
WHERE
    id = asset_solution_id;
...
SELECT
    id INTO status_assinatura_id
FROM
    asset_status
WHERE
    LOWER(status) = 'em assinatura'
LIMIT
    1;
...
FROM
    status_antigo THEN BEGIN -- Verificar se o status não foi alterado por outra transação concorrente
    DECLARE current_status_check BIGINT;

BEGIN
SELECT
    status_id INTO current_status_check
FROM
    assets
WHERE
    uuid = current_asset_id;
...
CREATE
OR REPLACE FUNCTION "public"."log_asset_insert"() RETURNS "trigger" LANGUAGE "plpgsql"
SET
    "search_path" TO 'public' AS $ $ BEGIN
INSERT INTO
    asset_logs (
...
CREATE
OR REPLACE FUNCTION "public"."log_asset_soft_delete"() RETURNS "trigger" LANGUAGE "plpgsql"
SET
    "search_path" TO '' AS $ $ DECLARE asset_solution_id BIGINT;
...
CREATE
OR REPLACE FUNCTION "public"."log_asset_status_change"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE last_assoc_id BIGINT;
...
RAISE NOTICE '[log_asset_status_change] Trigger disparada para asset uuid=%',
NEW.uuid;

-- Registra apenas se o status realmente mudou via UPDATE
IF NEW.status_id IS DISTINCT
FROM
    OLD.status_id
    AND TG_OP = 'UPDATE' THEN
SELECT
    solution_id INTO asset_solution_id
FROM
    assets
WHERE
    uuid = NEW.uuid;
...
CREATE
OR REPLACE FUNCTION "public"."log_client_changes"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE user_email TEXT;
...
CREATE
OR REPLACE FUNCTION "public"."log_profile_change"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE current_user_email TEXT;
...
IF safe_user_id IS NOT NULL THEN BEGIN
SELECT
    email INTO current_user_email
FROM
    auth.users
WHERE
    id = safe_user_id;
...
EXCEPTION
WHEN OTHERS THEN current_user_email := CASE
...
RAISE NOTICE 'log_profile_change: N\xC3\xA3o foi poss\xC3\xADvel acessar auth.users, usando email do registro: %',
current_user_email;

END;

ELSE current_user_email := CASE
    WHEN TG_OP = 'INSERT' THEN to_jsonb(NEW) ->> 'email'
    WHEN TG_OP = 'UPDATE' THEN COALESCE(to_jsonb(NEW) ->> 'email', to_jsonb(OLD) ->> 'email')
    WHEN TG_OP = 'DELETE' THEN to_jsonb(OLD) ->> 'email'
    ELSE 'sistema'
...
CREATE
OR REPLACE FUNCTION "public"."prevent_rented_association"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE current_status_id BIGINT;
...
SELECT
    a.status_id,
    ast.status INTO current_status_id,
    current_status_name
FROM
    assets a
...
IF current_status_id = status_alocado_id THEN RAISE EXCEPTION 'Não é possível associar o asset %, pois ele já está alocado (status: %).',
NEW.asset_id,
current_status_name;
...
CREATE
OR REPLACE FUNCTION "public"."release_operation_lock"("p_lock_id" "uuid") RETURNS boolean LANGUAGE "plpgsql"
SET
    "search_path" TO 'public, auth' AS $ $ DECLARE current_user_id UUID;
...
SET
    "search_path" TO '' AS $ $ BEGIN NEW.updated_at = NOW();
...
CREATE
OR REPLACE FUNCTION "public"."status_by_asset_type"() RETURNS TABLE("type" "text", "status" "text", "count" bigint) LANGUAGE "plpgsql"
SET
    "search_path" TO 'public' AS $ $ BEGIN RETURN QUERY
SELECT
    ac.solution AS type,
    -- text
...
CREATE
OR REPLACE FUNCTION "public"."update_all_rented_days"() RETURNS "jsonb" LANGUAGE "plpgsql"
SET
    "search_path" TO 'public, auth' AS $ $ DECLARE asset_record record;
...
BEGIN FOR asset_record IN (
    ...
    WHERE
        deleted_at IS NULL
) LOOP result := public.update_asset_rented_days(asset_record.uuid);

total_processed := total_processed + 1;

IF result ->> 'success' = 'true' THEN IF (result ->> 'updated') :: boolean THEN total_updated := total_updated + 1;
...
CREATE
OR REPLACE FUNCTION "public"."update_asset_rented_days"("asset_uuid" "text") RETURNS "jsonb" LANGUAGE "plpgsql"
SET
    "search_path" TO 'public, auth' AS $ $ DECLARE current_rented_days integer := 0;

calculated_days integer := 0;

new_total_days integer := 0;

BEGIN -- verificar se o ativo existe
IF NOT EXISTS (
    SELECT
        1
    FROM
...
-- valor historico
SELECT
    COALESCE(rented_days, 0) INTO current_rented_days
FROM
    public.assets
WHERE
    uuid = asset_uuid;
...
SELECT
    COUNT(DISTINCT d) :: integer INTO calculated_days
FROM
    (
        SELECT
            generate_series(entry_date, exit_date, INTERVAL '1 day') :: date AS d
        FROM
            public.asset_client_assoc
        WHERE
            asset_id = asset_uuid
...
RETURN jsonb_build_object(
    ...
    new_total_days,
    'updated',
    new_total_days >= current_rented_days
);
...
CREATE
OR REPLACE FUNCTION "public"."update_generic_updated_at_column"() RETURNS "trigger" LANGUAGE "plpgsql"
SET
    "search_path" TO 'public' AS $ $ BEGIN NEW.updated_at = NOW();
...
CREATE
OR REPLACE FUNCTION "public"."update_profile_last_login"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ BEGIN -- Atualizar o last_login no profiles quando o last_sign_in_at for atualizado
UPDATE
    public.profiles
SET
...
CREATE
OR REPLACE FUNCTION "public"."user_has_profile"("user_id" "uuid") RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ BEGIN RETURN EXISTS (
        SELECT
            1
        FROM
            profiles
...
CREATE
OR REPLACE FUNCTION "public"."validate_active_uuid"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE asset_count INT;
...
IF asset_count = 0 THEN RAISE EXCEPTION 'O asset_id % não existe na tabela assets.',
NEW.asset_id USING HINT = 'Certifique-se de cadastrar o asset antes de associá-lo a um cliente';
...
CREATE
OR REPLACE FUNCTION "public"."validate_association_state"(
    "p_asset_id" "text",
    "p_operation" "text",
    "p_association_id" bigint DEFAULT NULL :: bigint
) RETURNS "jsonb" LANGUAGE "plpgsql"
SET
    "search_path" TO 'public, auth' AS $ $ DECLARE asset_exists BOOLEAN := FALSE;
...
BEGIN -- Log detalhado dos parâmetros de entrada
...
OR trim(p_asset_id) = '' THEN RAISE NOTICE '[validate_association_state] ERRO: asset_id é obrigatório';
...
OR p_operation NOT IN ('CREATE', 'END') THEN RAISE NOTICE '[validate_association_state] ERRO: operation deve ser CREATE ou END, recebido: %',
p_operation;
...
RETURN jsonb_build_object(
    ...
    'INVALID_PARAMETERS',
    'message',
    'operation deve ser CREATE ou END',
    'operation',
    p_operation
);
...
RAISE NOTICE '[validate_association_state] Status IDs encontrados - disponível: %, alocado: %, assinatura: %', status_disponivel_id, status_alocado_id, status_assinatura_id;

-- Verificar se encontrou os status necessários
IF status_disponivel_id IS NULL THEN RAISE NOTICE '[validate_association_state] ERRO: Status disponível não encontrado';
...
RETURN jsonb_build_object(
    ...
    'STATUS_CONFIG_ERROR',
    'message',
    'Status disponível não encontrado no sistema'
);
...
SELECT
    CASE
        ...
        ELSE FALSE
    END,
    MAX(status_id) INTO asset_exists,
    asset_status_id
FROM
    assets
WHERE
...
RAISE NOTICE '[validate_association_state] Asset encontrado: %, status_id atual: %',
asset_exists,
asset_status_id;

EXCEPTION
WHEN OTHERS THEN RAISE NOTICE '[validate_association_state] ERRO ao buscar asset: %',
SQLERRM;
...
RETURN jsonb_build_object(
    ...
    'DATABASE_ERROR',
    'message',
    'Erro ao verificar existência do asset',
    'error_detail',
    SQLERRM,
    'asset_id',
    p_asset_id
...
IF NOT asset_exists THEN RAISE NOTICE '[validate_association_state] ERRO: Asset não encontrado - %',
p_asset_id;
...
BEGIN
SELECT
    status INTO asset_status_name
FROM
    asset_status
WHERE
    id = asset_status_id;

RAISE NOTICE '[validate_association_state] Nome do status atual: %',
asset_status_name;

EXCEPTION
WHEN OTHERS THEN RAISE NOTICE '[validate_association_state] ERRO ao buscar nome do status: %',
SQLERRM;
...
BEGIN
SELECT
    COUNT(*) INTO active_associations_count
FROM
    asset_client_assoc
WHERE
    asset_id = p_asset_id
...
RAISE NOTICE '[validate_association_state] Associações ativas encontradas: %',
active_associations_count;

EXCEPTION
WHEN OTHERS THEN RAISE NOTICE '[validate_association_state] ERRO ao contar associações ativas: %',
SQLERRM;
...
RETURN jsonb_build_object(
    ...
    'DATABASE_ERROR',
    'message',
    'Erro ao verificar associações ativas',
    'error_detail',
    SQLERRM,
    'asset_id',
    p_asset_id
...
IF p_operation = 'CREATE' THEN RAISE NOTICE '[validate_association_state] Validando operação CREATE';

-- Verificar se asset já está associado
IF active_associations_count > 0 THEN RAISE NOTICE '[validate_association_state] Asset já possui % associação(ões) ativa(s)',
active_associations_count;
...
RETURN jsonb_build_object(
    ...
    false,
    'error_code',
    'ASSET_ALREADY_ASSOCIATED',
    'message',
    'Asset já possui associação ativa',
    'asset_id',
    p_asset_id,
    'current_status',
    asset_status_name,
...
IF asset_status_id NOT IN (status_disponivel_id) THEN RAISE NOTICE '[validate_association_state] Status % não permite nova associação',
asset_status_name;
...
RETURN jsonb_build_object(
    ...
    false,
    'error_code',
    'INVALID_STATUS_FOR_ASSOCIATION',
    'message',
    'Status do asset não permite nova associação',
    'asset_id',
    p_asset_id,
    'current_status',
    asset_status_name,
...
ELSIF p_operation = 'END' THEN RAISE NOTICE '[validate_association_state] Validando operação END';

-- Verificar se existe associação para encerrar
IF active_associations_count = 0 THEN RAISE NOTICE '[validate_association_state] Nenhuma associação ativa encontrada para encerrar';

RETURN jsonb_build_object(
    'valid',
    false,
    'error_code',
    'NO_ACTIVE_ASSOCIATION',
    'message',
    'Não há associação ativa para encerrar',
    'asset_id',
    p_asset_id,
    'current_status',
    asset_status_name
...
IF p_association_id IS NOT NULL THEN IF NOT EXISTS (
    SELECT
        1
    FROM
        asset_client_assoc
    WHERE
        ...
        AND exit_date IS NULL
        AND deleted_at IS NULL
) THEN RAISE NOTICE '[validate_association_state] Associação específica % não encontrada',
p_association_id;
...
RETURN jsonb_build_object(
    ...
    'message',
    'Associação específica não encontrada ou já encerrada',
    'asset_id',
    p_asset_id,
    'association_id',
    p_association_id
...
RAISE NOTICE '[validate_association_state] Validação APROVADA para asset %',
p_asset_id;
...
RETURN jsonb_build_object(
    ...
    active_associations_count,
    'message',
    'Validação passou - operação pode prosseguir'
);

EXCEPTION
WHEN OTHERS THEN RAISE NOTICE '[validate_association_state] EXCEÇÃO CAPTURADA: % (SQLSTATE: %)',
SQLERRM,
SQLSTATE;
...
ALTER FUNCTION "public"."validate_association_state"(
    "p_asset_id" "text",
    "p_operation" "text",
...
CREATE
OR REPLACE FUNCTION "public"."validate_rented_days_integrity"() RETURNS TABLE(
    "asset_id" "text",
...
ORDER BY
    uuid
LIMIT
    10 -- Limitar para teste inicial
    LOOP -- Simular cálculo sem atualizar
    WITH periods AS (
        SELECT
            entry_date,
            exit_date
    ...
    merged AS (
        ...
        SELECT
            COALESCE(SUM(exit_date - entry_date + 1), 0) as total_days
        FROM
            periods
    )
SELECT
    total_days INTO calculated_days
FROM
    merged;
...
ALTER TABLE
    "public"."asset_client_assoc" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."asset_client_assoc_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
    "public"."asset_client_assoc_id_seq" OWNER TO "postgres";
...
ALTER TABLE
    "public"."asset_logs" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."asset_history_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
    "public"."asset_history_id_seq" OWNER TO "postgres";
...
ALTER TABLE
    "public"."asset_solutions" OWNER TO "postgres";
...
ALTER TABLE
    "public"."asset_status" OWNER TO "postgres";
...
ALTER TABLE
    "public"."assets" OWNER TO "postgres";
...
ALTER TABLE
    "public"."association_types" OWNER TO "postgres";
...
ADD
    GENERATED BY DEFAULT AS IDENTITY (
        SEQUENCE NAME "public"."association_types_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
    );
...
ALTER TABLE
    "public"."bits_badges_catalog" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_campaigns" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_levels_catalog" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_missions_catalog" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_points_log" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_referrals" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_rewards_catalog" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_user_badges" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_user_missions" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_user_profile_stats" OWNER TO "postgres";
...
ALTER TABLE
    "public"."bits_user_rewards" OWNER TO "postgres";
...
ALTER TABLE
    "public"."client_logs" OWNER TO "postgres";
...
ALTER TABLE
    "public"."clients" OWNER TO "postgres";
...
ALTER TABLE
    "public"."location_types" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."location_types_id_seq" AS integer START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
    "public"."location_types_id_seq" OWNER TO "postgres";
...
ALTER TABLE
    "public"."locations" OWNER TO "postgres";
...
ALTER TABLE
    "public"."locations_id_seq" OWNER TO "postgres";
...
ALTER TABLE
    "public"."manufacturers" OWNER TO "postgres";
...
ADD
    GENERATED BY DEFAULT AS IDENTITY (
        SEQUENCE NAME "public"."manufacturers_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
    );
...
ALTER TABLE
    "public"."operation_locks" OWNER TO "postgres";
...
CREATE SEQUENCE IF NOT EXISTS "public"."pacotes_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
    "public"."pacotes_id_seq" OWNER TO "postgres";
...
ALTER TABLE
    "public"."profile_logs" OWNER TO "postgres";
...
ALTER TABLE
    "public"."asset_solutions"
ALTER COLUMN
    "id"
ADD
    GENERATED BY DEFAULT AS IDENTITY (
        SEQUENCE NAME "public"."solution_types_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
    );
...
ADD
    GENERATED BY DEFAULT AS IDENTITY (
        SEQUENCE NAME "public"."status_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
    );

CREATE
OR REPLACE VIEW "public"."v_active_clients" WITH ("security_invoker" = 'on') AS
SELECT
    DISTINCT "asset_client_assoc"."client_id"
FROM
    "public"."asset_client_assoc"
WHERE
    (
...
ALTER TABLE
    "public"."v_active_clients" OWNER TO "postgres";

CREATE
OR REPLACE VIEW "public"."v_problem_assets" WITH ("security_invoker" = 'on') AS
SELECT
    "a"."uuid",
    "a"."solution_id",
    "a"."line_number",
...
ALTER TABLE
    "public"."v_problem_assets" OWNER TO "postgres";

ALTER TABLE
    ONLY "public"."asset_client_assoc"
ALTER COLUMN
    "id"
SET
    DEFAULT "nextval"(
        '"public"."asset_client_assoc_id_seq"' :: "regclass"
    );

ALTER TABLE
    ONLY "public"."asset_logs"
ALTER COLUMN
    "id"
SET
    DEFAULT "nextval"('"public"."asset_history_id_seq"' :: "regclass");

ALTER TABLE
    ONLY "public"."location_types"
ALTER COLUMN
    "id"
SET
    DEFAULT "nextval"('"public"."location_types_id_seq"' :: "regclass");
...
ALTER COLUMN
    "id"
SET
    DEFAULT "nextval"('"public"."pacotes_id_seq"' :: "regclass");
...
ALTER TABLE
    ONLY "public"."bits_badges_catalog"
ADD
    CONSTRAINT "bits_badges_catalog_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_campaigns"
ADD
    CONSTRAINT "bits_campaigns_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_levels_catalog"
ADD
    CONSTRAINT "bits_levels_catalog_pkey" PRIMARY KEY ("level_number");

ALTER TABLE
    ONLY "public"."bits_missions_catalog"
ADD
    CONSTRAINT "bits_missions_catalog_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_points_log"
ADD
    CONSTRAINT "bits_points_log_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_referrals"
ADD
    CONSTRAINT "bits_referrals_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_rewards_catalog"
ADD
    CONSTRAINT "bits_rewards_catalog_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_user_badges"
ADD
    CONSTRAINT "bits_user_badges_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_user_badges"
ADD
    CONSTRAINT "bits_user_badges_user_id_badge_id_key" UNIQUE ("user_id", "badge_id");

ALTER TABLE
    ONLY "public"."bits_user_missions"
ADD
    CONSTRAINT "bits_user_missions_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."bits_user_missions"
ADD
    CONSTRAINT "bits_user_missions_user_id_mission_id_key" UNIQUE ("user_id", "mission_id");

ALTER TABLE
    ONLY "public"."bits_user_profile_stats"
ADD
    CONSTRAINT "bits_user_profile_stats_pkey" PRIMARY KEY ("user_id");

ALTER TABLE
    ONLY "public"."bits_user_rewards"
ADD
    CONSTRAINT "bits_user_rewards_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."client_logs"
ADD
...
ALTER TABLE
    ONLY "public"."manufacturers"
ADD
    CONSTRAINT "manufacturers_pkey" PRIMARY KEY ("id");

ALTER TABLE
    ONLY "public"."operation_locks"
ADD
...
CREATE UNIQUE INDEX "idx_assets_radio_unique_not_null" ON "public"."assets" USING "btree" ("radio")
WHERE
    ("radio" IS NOT NULL);
...
CREATE
OR REPLACE TRIGGER "asset_client_assoc_log_trigger"
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON "public"."asset_client_assoc" FOR EACH ROW EXECUTE FUNCTION "public"."log_profile_change"();

CREATE
OR REPLACE TRIGGER "asset_client_assoc_status_log_trigger"
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON "public"."asset_client_assoc" FOR EACH ROW EXECUTE FUNCTION "public"."log_and_update_status"();

CREATE
OR REPLACE TRIGGER "asset_histories_log_trigger"
AFTER
INSERT
    OR
UPDATE
    ON "public"."asset_logs" FOR EACH ROW EXECUTE FUNCTION "public"."log_profile_change"();

CREATE
OR REPLACE TRIGGER "asset_histories_log_trigger_before_delete" BEFORE DELETE ON "public"."asset_logs" FOR EACH ROW EXECUTE FUNCTION "public"."log_profile_change"();

CREATE
OR REPLACE TRIGGER "asset_status_update_history"
AFTER
UPDATE
    OF "status_id" ON "public"."assets" FOR EACH ROW EXECUTE FUNCTION "public"."log_asset_status_change"();

CREATE
OR REPLACE TRIGGER "assets_after_insert"
AFTER
INSERT
    ON "public"."assets" FOR EACH ROW EXECUTE FUNCTION "public"."log_asset_insert"();

CREATE
OR REPLACE TRIGGER "assets_log_trigger"
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON "public"."assets" FOR EACH ROW EXECUTE FUNCTION "public"."log_profile_change"();

CREATE
OR REPLACE TRIGGER "check_availability_before_association" BEFORE
INSERT
    OR
UPDATE
    ON "public"."asset_client_assoc" FOR EACH ROW EXECUTE FUNCTION "public"."check_asset_availability"();

CREATE
OR REPLACE TRIGGER "client_changes_trigger"
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON "public"."clients" FOR EACH ROW EXECUTE FUNCTION "public"."log_client_changes"();

CREATE
OR REPLACE TRIGGER "client_logs_updated_at_trigger" BEFORE
UPDATE
    ON "public"."client_logs" FOR EACH ROW EXECUTE FUNCTION "public"."update_generic_updated_at_column"();

CREATE
OR REPLACE TRIGGER "clients_log_trigger"
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON "public"."clients" FOR EACH ROW EXECUTE FUNCTION "public"."log_profile_change"();

CREATE
OR REPLACE TRIGGER "handle_bits_referrals_updated_at" BEFORE
UPDATE
    ON "public"."bits_referrals" FOR EACH ROW EXECUTE FUNCTION "public"."update_generic_updated_at_column"();

CREATE
OR REPLACE TRIGGER "handle_bits_rewards_catalog_updated_at" BEFORE
UPDATE
    ON "public"."bits_rewards_catalog" FOR EACH ROW EXECUTE FUNCTION "public"."update_generic_updated_at_column"();

CREATE
OR REPLACE TRIGGER "log_and_update_status_on_status_change"
AFTER
INSERT
    OR
UPDATE
...
CREATE
OR REPLACE TRIGGER "plans_log_trigger"
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON "public"."plans" FOR EACH ROW EXECUTE FUNCTION "public"."log_profile_change"();

CREATE
OR REPLACE TRIGGER "prevent_rented_association_before" BEFORE
INSERT
    ON "public"."asset_client_assoc" FOR EACH ROW EXECUTE FUNCTION "public"."prevent_rented_association"();

CREATE
OR REPLACE TRIGGER "profiles_log_trigger"
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON "public"."profiles" FOR EACH ROW EXECUTE FUNCTION "public"."log_profile_change"();

CREATE
OR REPLACE TRIGGER "set_asset_client_assoc_updated_at" BEFORE
UPDATE
    ON "public"."asset_client_assoc" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_asset_logs_updated_at" BEFORE
UPDATE
    ON "public"."asset_logs" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_asset_solutions_updated_at" BEFORE
UPDATE
    ON "public"."asset_solutions" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_asset_status_updated_at" BEFORE
UPDATE
    ON "public"."asset_status" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_assets_updated_at" BEFORE
UPDATE
    ON "public"."assets" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_association_types_updated_at" BEFORE
UPDATE
    ON "public"."association_types" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_badges_catalog_updated_at" BEFORE
UPDATE
    ON "public"."bits_badges_catalog" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_campaigns_updated_at" BEFORE
UPDATE
    ON "public"."bits_campaigns" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_levels_catalog_updated_at" BEFORE
UPDATE
    ON "public"."bits_levels_catalog" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_missions_catalog_updated_at" BEFORE
UPDATE
    ON "public"."bits_missions_catalog" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_points_log_updated_at" BEFORE
UPDATE
    ON "public"."bits_points_log" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_referrals_updated_at" BEFORE
UPDATE
    ON "public"."bits_referrals" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_rewards_catalog_updated_at" BEFORE
UPDATE
    ON "public"."bits_rewards_catalog" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_user_badges_updated_at" BEFORE
UPDATE
    ON "public"."bits_user_badges" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_user_missions_updated_at" BEFORE
UPDATE
    ON "public"."bits_user_missions" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_user_profile_stats_updated_at" BEFORE
UPDATE
    ON "public"."bits_user_profile_stats" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_bits_user_rewards_updated_at" BEFORE
UPDATE
    ON "public"."bits_user_rewards" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_clients_updated_at" BEFORE
UPDATE
    ON "public"."clients" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_location_types_updated_at" BEFORE
UPDATE
    ON "public"."location_types" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_locations_updated_at" BEFORE
UPDATE
    ON "public"."locations" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_manufacturers_updated_at" BEFORE
UPDATE
    ON "public"."manufacturers" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_plans_updated_at" BEFORE
UPDATE
    ON "public"."plans" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();

CREATE
OR REPLACE TRIGGER "set_profile_logs_updated_at" BEFORE
UPDATE
    ON "public"."profile_logs" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();
...
CREATE
OR REPLACE TRIGGER "trg_log_asset_soft_delete"
AFTER
UPDATE
    ON "public"."assets" FOR EACH ROW
    WHEN (
        (
            ("old"."deleted_at" IS NULL)
            AND ("new"."deleted_at" IS NOT NULL)
        )
    ) EXECUTE FUNCTION "public"."log_asset_soft_delete"();

CREATE
OR REPLACE TRIGGER "trigger_log_profile_changes"
AFTER
INSERT
    OR DELETE
    OR
...
CREATE
OR REPLACE TRIGGER "update_locations_updated_at" BEFORE
UPDATE
    ON "public"."locations" FOR EACH ROW EXECUTE FUNCTION "public"."update_generic_updated_at_column"();

CREATE
OR REPLACE TRIGGER "update_manufacturer_updated_at" BEFORE
UPDATE
    ON "public"."manufacturers" FOR EACH ROW EXECUTE FUNCTION "public"."update_generic_updated_at_column"();

CREATE
OR REPLACE TRIGGER "validate_active_uuid_on_change" BEFORE
INSERT
    OR
UPDATE
    ON "public"."asset_client_assoc" FOR EACH ROW EXECUTE FUNCTION "public"."validate_active_uuid"();

ALTER TABLE
    ONLY "public"."asset_client_assoc"
ADD
    CONSTRAINT "asset_client_assoc_asset_id_fkey" FOREIGN KEY ("asset_id") REFERENCES "public"."assets"("uuid") ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."asset_client_assoc"
ADD
    CONSTRAINT "asset_client_assoc_client_id_fkey" FOREIGN KEY ("client_id") REFERENCES "public"."clients"("uuid") ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."asset_client_assoc"
ADD
    CONSTRAINT "asset_client_assoc_plan_id_fkey" FOREIGN KEY ("plan_id") REFERENCES "public"."plans"("id") ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."assets"
ADD
    CONSTRAINT "assets_manufacturer_id_fkey" FOREIGN KEY ("manufacturer_id") REFERENCES "public"."manufacturers"("id") ON UPDATE CASCADE ON DELETE
SET
    NULL;
...
ALTER TABLE
    ONLY "public"."assets"
ADD
    CONSTRAINT "assets_plan_id_fkey" FOREIGN KEY ("plan_id") REFERENCES "public"."plans"("id") ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_missions_catalog"
ADD
    CONSTRAINT "bits_missions_catalog_badge_reward_id_fkey" FOREIGN KEY ("badge_reward_id") REFERENCES "public"."bits_badges_catalog"("id");

ALTER TABLE
    ONLY "public"."bits_points_log"
ADD
    CONSTRAINT "bits_points_log_related_referral_id_fkey" FOREIGN KEY ("related_referral_id") REFERENCES "public"."bits_referrals"("id") ON DELETE
SET
    NULL;
...
ALTER TABLE
    ONLY "public"."bits_points_log"
ADD
    CONSTRAINT "bits_points_log_related_reward_id_fkey" FOREIGN KEY ("related_reward_id") REFERENCES "public"."bits_rewards_catalog"("id") ON DELETE
SET
    NULL;
...
ALTER TABLE
    ONLY "public"."bits_points_log"
ADD
    CONSTRAINT "bits_points_log_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_referrals"
ADD
    CONSTRAINT "bits_referrals_referrer_user_id_fkey" FOREIGN KEY ("referrer_user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_user_badges"
ADD
    CONSTRAINT "bits_user_badges_badge_id_fkey" FOREIGN KEY ("badge_id") REFERENCES "public"."bits_badges_catalog"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_user_badges"
ADD
    CONSTRAINT "bits_user_badges_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_user_missions"
ADD
    CONSTRAINT "bits_user_missions_mission_id_fkey" FOREIGN KEY ("mission_id") REFERENCES "public"."bits_missions_catalog"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_user_missions"
ADD
    CONSTRAINT "bits_user_missions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_user_profile_stats"
ADD
    CONSTRAINT "bits_user_profile_stats_current_level_number_fkey" FOREIGN KEY ("current_level_number") REFERENCES "public"."bits_levels_catalog"("level_number");

ALTER TABLE
    ONLY "public"."bits_user_profile_stats"
ADD
    CONSTRAINT "bits_user_profile_stats_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."bits_user_rewards"
ADD
    CONSTRAINT "bits_user_rewards_reward_id_fkey" FOREIGN KEY ("reward_id") REFERENCES "public"."bits_rewards_catalog"("id") ON DELETE RESTRICT;

ALTER TABLE
    ONLY "public"."bits_user_rewards"
ADD
    CONSTRAINT "bits_user_rewards_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;

ALTER TABLE
    ONLY "public"."asset_logs"
ADD
    CONSTRAINT "fk_asset_history_status_after" FOREIGN KEY ("status_after_id") REFERENCES "public"."asset_status"("id");

ALTER TABLE
    ONLY "public"."asset_logs"
ADD
    CONSTRAINT "fk_asset_history_status_before" FOREIGN KEY ("status_before_id") REFERENCES "public"."asset_status"("id");

ALTER TABLE
    ONLY "public"."asset_logs"
ADD
    CONSTRAINT "fk_asset_logs_assoc_id" FOREIGN KEY ("assoc_id") REFERENCES "public"."asset_client_assoc"("id") ON DELETE
SET
    NULL;
...
ALTER TABLE
    ONLY "public"."asset_logs"
ADD
    CONSTRAINT "fk_asset_logs_status_after" FOREIGN KEY ("status_after_id") REFERENCES "public"."asset_status"("id");

ALTER TABLE
    ONLY "public"."asset_logs"
ADD
    CONSTRAINT "fk_asset_logs_status_before" FOREIGN KEY ("status_before_id") REFERENCES "public"."asset_status"("id");

ALTER TABLE
    ONLY "public"."assets"
ADD
    CONSTRAINT "fk_assets_solutions" FOREIGN KEY ("solution_id") REFERENCES "public"."asset_solutions"("id");

ALTER TABLE
    ONLY "public"."assets"
ADD
    CONSTRAINT "fk_assets_status" FOREIGN KEY ("status_id") REFERENCES "public"."asset_status"("id");

ALTER TABLE
    ONLY "public"."asset_client_assoc"
ADD
    CONSTRAINT "fk_assoc_association_type" FOREIGN KEY ("association_id") REFERENCES "public"."association_types"("id");

ALTER TABLE
    ONLY "public"."locations"
ADD
    CONSTRAINT "locations_client_id_fkey" FOREIGN KEY ("client_id") REFERENCES "public"."clients"("uuid") ON UPDATE CASCADE ON DELETE
SET
    NULL;
...
ALTER TABLE
    ONLY "public"."locations"
ADD
    CONSTRAINT "locations_type_id_fkey" FOREIGN KEY ("type_id") REFERENCES "public"."location_types"("id");
...
ALTER TABLE
    ONLY "public"."asset_status"
ADD
    CONSTRAINT "status_association_fkey" FOREIGN KEY ("association") REFERENCES "public"."association_types"("id") ON UPDATE CASCADE ON DELETE
SET
    NULL;

CREATE POLICY "Admins can do everything with referrals" ON "public"."bits_referrals" TO "authenticated" USING (
    "public"."has_role"('admin' :: "public"."user_role_enum")
);

CREATE POLICY "Admins can insert profile logs" ON "public"."profile_logs" FOR
INSERT
...
CREATE POLICY "Admins can view all profile logs" ON "public"."profile_logs" FOR
SELECT
    TO "authenticated" USING (
        "public"."has_role"('admin' :: "public"."user_role_enum")
    );

CREATE POLICY "Afiliados can manage their own redeemed rewards" ON "public"."bits_user_rewards" USING (
    (
        ("user_id" = "auth"."uid"())
        AND "public"."is_afiliado"()
    )
) WITH CHECK (
    (
        ("user_id" = "auth"."uid"())
...
CREATE POLICY "Afiliados can manage their own referrals" ON "public"."bits_referrals" USING (
    (
...
CREATE POLICY "Afiliados can view their own points log" ON "public"."bits_points_log" FOR
SELECT
    USING (
        (
            ("user_id" = "auth"."uid"())
...
CREATE POLICY "Allow profile_logs insertion by security definer functions" ON "public"."profile_logs" FOR
INSERT
    WITH CHECK (true);

CREATE POLICY "Auth users can insert" ON "public"."asset_client_assoc" FOR
INSERT
    TO "authenticated" WITH CHECK (true);

CREATE POLICY "Auth users can insert" ON "public"."asset_logs" FOR
INSERT
    TO "authenticated" WITH CHECK (true);

CREATE POLICY "Auth users can insert" ON "public"."assets" FOR
INSERT
    TO "authenticated" WITH CHECK (true);

CREATE POLICY "Auth users can insert" ON "public"."client_logs" FOR
INSERT
    TO "authenticated" WITH CHECK (true);

CREATE POLICY "Auth users can insert" ON "public"."clients" FOR
INSERT
    TO "authenticated" WITH CHECK (true);

CREATE POLICY "Auth users can update" ON "public"."asset_client_assoc" FOR
UPDATE
    TO "authenticated" USING (true);

CREATE POLICY "Auth users can update" ON "public"."asset_logs" FOR
UPDATE
    TO "authenticated" USING (true);

CREATE POLICY "Auth users can update" ON "public"."assets" FOR
UPDATE
    TO "authenticated" USING (true) WITH CHECK (true);

CREATE POLICY "Auth users can update" ON "public"."client_logs" FOR
UPDATE
    TO "authenticated" USING (true);

CREATE POLICY "Auth users can update" ON "public"."clients" FOR
UPDATE
    TO "authenticated" USING (true);

CREATE POLICY "Authenticated users can read active campaigns" ON "public"."bits_campaigns" FOR
SELECT
    TO "authenticated" USING (("is_active" = true));

CREATE POLICY "Authenticated users can read badges catalog" ON "public"."bits_badges_catalog" FOR
SELECT
    TO "authenticated" USING (true);

CREATE POLICY "Authenticated users can read levels catalog" ON "public"."bits_levels_catalog" FOR
SELECT
    TO "authenticated" USING (true);

CREATE POLICY "Authenticated users can read missions catalog" ON "public"."bits_missions_catalog" FOR
SELECT
    TO "authenticated" USING (true);

CREATE POLICY "Authenticated users can view rewards catalog" ON "public"."bits_rewards_catalog" FOR
SELECT
    USING (("auth"."role"() = 'authenticated' :: "text"));

CREATE POLICY "Clientes can create referrals" ON "public"."bits_referrals" FOR
INSERT
    TO "authenticated" WITH CHECK (("referrer_user_id" = "auth"."uid"()));

CREATE POLICY "Clientes can view their own referrals" ON "public"."bits_referrals" FOR
SELECT
    TO "authenticated" USING (("referrer_user_id" = "auth"."uid"()));

CREATE POLICY "Enable read access for all users" ON "public"."asset_client_assoc" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."asset_logs" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."asset_solutions" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."asset_status" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."assets" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."association_types" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."client_logs" FOR
SELECT
    USING (true);
...
CREATE POLICY "Enable read access for all users" ON "public"."location_types" FOR
SELECT
    TO "authenticated" USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."locations" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."manufacturers" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."plans" FOR
SELECT
    USING (true);

CREATE POLICY "Enable read your data access for all users" ON "public"."profiles" FOR
SELECT
    USING ("public"."is_user_self"("id"));

CREATE POLICY "Enable update for users based on email" ON "public"."profiles" USING (
    (
        (
            (
                SELECT
...
CREATE POLICY "Only admins can view profile logs" ON "public"."profile_logs" FOR
SELECT
    USING (
        (
            EXISTS (
                ...
                WHERE
                    (
                        ...
                        AND (
                            "profiles"."role" = 'admin' :: "public"."user_role_enum"
                        )
                    )
            )
        )
...
CREATE POLICY "Suporte can manage all referrals" ON "public"."bits_referrals" TO "authenticated" USING (
    "public"."has_role"('suporte' :: "public"."user_role_enum")
);

CREATE POLICY "Users can create locks or admins can create any" ON "public"."operation_locks" FOR
INSERT
    TO "authenticated" WITH CHECK (
        (
            ("user_id" = "auth"."uid"())
            OR "public"."has_minimum_role"('admin' :: "public"."user_role_enum")
        )
    );

CREATE POLICY "Users can delete their own locks or admins can delete any" ON "public"."operation_locks" FOR DELETE TO "authenticated" USING (
    (
        ("user_id" = "auth"."uid"())
        OR "public"."has_minimum_role"('admin' :: "public"."user_role_enum")
    )
...
CREATE POLICY "Users can update their own locks or admins can update any" ON "public"."operation_locks" FOR
UPDATE
    TO "authenticated" USING (
        (
            ("user_id" = "auth"."uid"())
...
CREATE POLICY "Users can update their own mission progress" ON "public"."bits_user_missions" FOR
UPDATE
    USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can view their own earned badges" ON "public"."bits_user_badges" FOR
SELECT
    USING (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can view their own locks or admins can view all" ON "public"."operation_locks" FOR
SELECT
    TO "authenticated" USING (
        (
            ("user_id" = "auth"."uid"())
...
CREATE POLICY "Users can view their own missions" ON "public"."bits_user_missions" FOR
SELECT
    USING (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can view their own profile stats" ON "public"."bits_user_profile_stats" FOR
SELECT
    USING (("auth"."uid"() = "user_id"));
...
ALTER TABLE
    "public"."asset_client_assoc" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."asset_logs" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."asset_solutions" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."asset_status" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."assets" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."association_types" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_badges_catalog" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_campaigns" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_levels_catalog" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_missions_catalog" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_points_log" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_referrals" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_rewards_catalog" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_user_badges" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_user_missions" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_user_profile_stats" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."bits_user_rewards" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."client_logs" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."clients" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."location_types" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."locations" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."manufacturers" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."operation_locks" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."plans" ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    "public"."profile_logs" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."acquire_operation_lock"(
    "p_operation_type" "text",
    "p_resource_id" "text",
    "p_operation_data" "jsonb",
    "p_timeout_minutes" integer
...
GRANT ALL ON FUNCTION "public"."admin_update_user_role"(
    "user_id" "uuid",
    "new_role" "public"."user_role_enum"
) TO "anon";
...
GRANT ALL ON FUNCTION "public"."ensure_user_profile"(
    "user_id" "uuid",
    "user_email" "text",
    "user_role" "public"."user_role_enum"
) TO "anon";
...
GRANT ALL ON FUNCTION "public"."validate_association_state"(
    "p_asset_id" "text",
    "p_operation" "text",
    "p_association_id" bigint
) TO "anon";
